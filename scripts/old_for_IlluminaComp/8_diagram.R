...
digraph workflow {
  ...
  # Default node style with larger font
  ...
  S15 -> S18 [
    label = 'amp_size < 200 bp',
    tailport = w,
    headport = w
  ]
  S15 -> S16 [label = 'amp_size >= 200 bp']
  S16 -> S17 [label = 'marker = COI-5P\nAND\namp_size >= 300 bp']
  S16 -> S18
  S17 -> S18
  
  # === Offshoot output from Demultiplex ===
  S4 -> S5 [style = dashed, color = gray40]
  
  # Force S5 to appear horizontally beside S4
  { rank = same; S4; S5 }
...
# Optionally, convert SVG to PNG with larger dimensions
